...
- 코딩 모드의 승인 전 계획 단계에서는 세 모델 모두 읽기 전용, 승인 후 실행 단계에서는 세 모델 모두 읽기·쓰기로 동작합니다.
- **무제한 코딩**: 사용자가 중단할 때까지 구현, 자동 검증, 교차 검토와 토론 결과 통합을 계속 반복합니다.
...
무제한 코딩은 다음 사이클을 종료 조건 없이 반복합니다.
...
모델이 완성됐다고 판단해도 품질, 테스트, 접근성, 성능과 운영 완성도를 다시 점검하며 다음 사이클을 계속합니다. 대시보드의 `중단`을 누르면 현재 CLI 프로세스와 반복을 종료합니다. 무제한 모드에서는 자동 토큰·비용 한도로 반복을 멈추지 않으며, CLI 실패나 역할 범위 위반 시에는 검토를 위해 일시정지합니다.
